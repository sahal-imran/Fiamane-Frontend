...
function CountryCodeDropdown({ State, Event, GetValueEvent, Placeholder = "Select", Item, Styles = "w-full h-[48px] rounded-[8px] border-[1px] border-[#E6E6E6]" }: Props) {
        ...
        <div className='w-full absolute top-[105%] z-50 left-0 drop-shadow-xl bg-white-main rounded-bl-[8px] rounded-br-[8px] py-1 max-h-[120px] overflow-auto CountryCodeDropdown' >
          {
            Item.map((item: any, index: number) => {
              return <div key={index} onClick={() => { SetValueTitle(item.phone); GetValueEvent(item.phone); Event(!State) }} className={`w-full font-OpenSans font-[400] text-black-main text-[16px] leading-[30px] text-start px-4 ${ValueTitle === item.phone && "bg-brand-mainCool"} hover:bg-brand-mainCool cursor-pointer`} >
                {item.phone}
              </div>
            })
          }
